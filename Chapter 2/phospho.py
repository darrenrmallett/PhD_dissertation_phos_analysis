...
import os, sys, filereader as fr, urllib.parse, mass_spec as ms, re, proteins, compare_files, time, PSSM as pssm
...
while start_over == "YES":
    ...
    print('Place Excel (.xlsx) file(s) that you want to analyze and compare in folder "msfiles". The program will analyze and compare all files in this folder. Make sure to remove old files you do not want analyzed.\nContinue? Type Y')
    cont = input()
...
min_phos_conf_input = input().strip()
