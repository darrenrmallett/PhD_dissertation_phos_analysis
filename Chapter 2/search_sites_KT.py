...
output_file_name = 'all_KT_' + kinase + '_' + str(unix) + '.csv'

for protein, info in kinetochore.Kinetochore().accessions.items():
    
    protein = proteins.Protein(info)
...
with open(current_output_path + '/' + output_file_name, 'w') as filehandle:
    ...
    for protein_name, info in scores_above_1.items():
        for residue, res_info in info.items():
            ...
            filehandle.write(f"{protein_name}\t{residue}\t{res_info['pep']}\t{res_info['o_r']}\t{res_info['rank_num']}\t{res_info['rank_list_str']}\n")